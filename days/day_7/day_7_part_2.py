...
class Day7Part2(Day):
    ...
    def solve(self):
        ...
        used_space = self.size_of(ls_results, Path('/'))
        self.print_answer(
            min(
                (directory for directory in ls_results.values() if used_space + REQUIRED_UPDATE_DISKS_SPACE - directory['size'] <= DISK_SPACE),
